...
- Time Complexity:
  - We iterate through the string once. For each character, we perform a constant time operation (adding it to the set), 
    which takes O(1) time. Therefore, the time complexity is O(n), where n is the length of the sentence.
